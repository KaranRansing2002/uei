...
const Semester = ({ aggOption, sem, dispatch }) => {
    ...
    const handleAddSubject = () => {
        subjects.current.push({ subject: '', [aggOption]: '' })
    ...
    const handleChange = (e, index) => {
        subjects.current[index][aggOption] = e.target.value;
        if (aggOption != 'grade') {
            let aggr = 0;
            subjects.current.map(obj => { aggr += parseInt(obj[aggOption]); });
            // console.log(aggr);
...
function MarksheetForm({ stateinfo, info }) {
    ...
    const [state, dispatch] = useReducer(reducer, { Class: '', aggregate: '', semesters: [{}, {}],Date : new Date().toISOString().substr(0, 10) });
